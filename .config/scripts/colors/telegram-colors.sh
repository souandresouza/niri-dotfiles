#!/usr/bin/env bash
#
# A script to create Telegram palettes that use colors generated by ruwall/wal
...
# Set colors from 9 to 16 for wal16
...
prepare() {
  ...
  if [[ "$bg" == "wal" ]]; then # if no background is specified
    if [[ "$mode" == wal ]] && [[ "$bg_mode" == "background" ]]; then
      ...
      bg_path="$HOME/.cache/current_wallpaper.png"
      bg_file="${bg_path##*/}"
